...
class ASTListener(AssignmentStatementListener):
    ...
    def exitWhilest(self, ctx: AssignmentStatementParser.WhilestContext):
        condition = ctx.cond().value_attr
        loopBody = ctx.statement().value_attr
        loopNode = self.ast.make_node(value='while',child=condition, brother=None)
        self.ast.add_child(loopNode,loopBody)
        ctx.value_attr = loopNode
        self.ast.root = loopNode
