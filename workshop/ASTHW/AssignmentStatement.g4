...
statement  returns [value_attr = str(), type_attr = str()]: ifst | assign | forst | whilest |compoundst ;

ifst  returns [value_attr = str(), type_attr = str()]: 'if' cond 'then' statement ('else' statement)? ;

forst returns [value_attr = str(), type_attr = str()]: 'for' ID ':=' expr 'to' expr 'do' statement ;

whilest returns [value_attr = str(), type_attr = str()]: 'while' cond 'do' statement ;
