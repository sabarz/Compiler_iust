class ILMapper:
    ...
    def binary_operator(self, a, b, item):
        ...
        if item == "<=":
            return self.lower_or_equal(a, b)
        if item == "!=":
    ...
    def greater_or_equal(self, a, b):
        ...
        x = 'clt \n'
        y = 'ceq \n'
        result = first_load_statement + second_load_statement + x + "ldc.i4 0 \n" + y
        self.push_temporary_to_stack()
        return result

    def lower_or_equal(self, a, b):
        if not self.is_temporary_operand(b):
            if self.is_identifier(b):
                second_load_statement = f"ldloc {b}\n"
            else:
    ...
    def for_statement(self):
        ...
        part1 = temp_for_statement.pop()

        loop_var = ''
        for index in part1.split('\n'):
            if 'stloc' in index:
                variable = index[6:]
        ...
        expresion = self.stack.pop()

        if self.is_temporary_operand(expresion):
            result = result + temp_for_statement.pop()
        else:
            if self.is_identifier(expresion):
                result = result + f"ldloc {expresion}\n"
            else:
                result = result + f"ldc.i8 {expresion}\n"

        result = result + f'ldloc {variable}\n'
