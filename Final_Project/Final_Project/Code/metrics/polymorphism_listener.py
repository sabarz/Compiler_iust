...
class PolymorphismListener(JavaParserListener):
    def __init__(self):
        self.class_list = []
        self.interface_list = []
    ...
    def enterClassBodyDeclaration2(self, ctx:JavaParser.ClassBodyDeclaration):
        # we only care about method modifiers of classes
        if not (
            isinstance(ctx.memberDeclaration(), JavaParser.MemberDeclaration0Context)
            or isinstance(ctx.memberDeclaration(), JavaParser.MemberDeclaration1Context)
        ):
            return
    ...
    def enterInterfaceBodyDeclaration(self, ctx:JavaParser.InterfaceBodyDeclarationContext):
        ...
        if not isinstance(ctx.interfaceMemberDeclaration(), JavaParser.InterfaceMemberDeclaration1Context):
            return
        modifier = JavaModifier()
        for m in ctx.modifier():
            if m.classOrInterfaceModifier().PRIVATE():
